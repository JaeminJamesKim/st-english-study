...
st.divider()
st.write(f" 소모임 어플에서 참여 멤버 리스트를 캡쳐 후,") 
st.write(f" 이미지들을 업로드 해주세요.")
st.write(f" 1부, 2부 종료 시, 반드시 각각 Update Attendance를 실행해 주세요.")
st.write(f" 1부 종료 --> Update Attendance")
st.write(f" 2부 종료 --> Update Attendance")
st.write(f" ")
st.write(f" * 1부만 참여하는 인원: ")
